...
python ${NEMO_LAUNCH_SCRIPT} --factory "recipe(profile_enabled=True,profile_start_step=7,profile_end_step=8,profile_ranks='0')" \
trainer.num_nodes="$NNODES" \
log.explicit_log_dir="${explicit_log_dir}" \
trainer.max_steps=10 trainer.num_nodes=32 trainer.devices=8 \
...
${NEMO_LAUNCH_SCRIPT} --factory "recipe(profile_enabled=True,profile_start_step=7,profile_end_step=8,profile_ranks='0')" \
trainer.num_nodes="$NNODES" \
log.explicit_log_dir="${explicit_log_dir}" \
trainer.max_steps=10 trainer.num_nodes=32 trainer.devices=8
